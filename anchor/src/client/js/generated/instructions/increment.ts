...
import { LENDINGDAPP_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof LENDINGDAPP_PROGRAM_ADDRESS,
  TAccountLendingdapp extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountLendingdapp extends string
        ? WritableAccount<TAccountLendingdapp>
        : TAccountLendingdapp,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountLendingdapp extends string = string> = {
  lendingdapp: Address<TAccountLendingdapp>;
};

export function getIncrementInstruction<
  TAccountLendingdapp extends string,
  TProgramAddress extends Address = typeof LENDINGDAPP_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountLendingdapp>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountLendingdapp> {
  // Program address.
  const programAddress = config?.programAddress ?? LENDINGDAPP_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    lendingdapp: { value: input.lendingdapp ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.lendingdapp)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountLendingdapp>;
